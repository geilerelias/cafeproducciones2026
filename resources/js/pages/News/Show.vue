    ...
    <MarketingLayout :title="news.title" :description="news.meta_description || news.excerpt" :image="news.image_url || assets.projectImage">
        <article>
            <section class="relative isolate overflow-hidden bg-zinc-950 py-24 text-white">
                ...
                <div class="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                    <div class="flex flex-wrap gap-2">
                        <span class="rounded-full bg-[#f0c8be] px-3 py-1 text-xs font-black uppercase tracking-wide text-[#7f241f]">{{ news.platform || news.category }}</span>
                        <span class="rounded-full border border-white/20 px-3 py-1 text-xs font-black uppercase tracking-wide">{{ news.published_at || 'Publicado' }}</span>
                    </div>
            ...
            <section class="mx-auto max-w-3xl px-4 py-14 sm:px-6 lg:px-8">
                ...
                <a v-if="news.source_url" :href="news.source_url" target="_blank" rel="noopener" class="mt-8 inline-flex rounded-md bg-[#a8322b] px-5 py-3 text-sm font-black text-white">
