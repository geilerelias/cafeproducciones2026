    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <main class="min-h-screen overflow-x-hidden bg-zinc-50 p-3 sm:p-4 md:p-6">
            <section class="rounded-md bg-zinc-950 p-5 text-white shadow-xl sm:p-6">
                ...
                <p class="mt-3 max-w-3xl text-sm leading-6 text-zinc-300">Crea permisos atomicos y luego asignales roles desde la pantalla de roles.</p>
            </section>

            <section class="mt-6 grid gap-6 xl:grid-cols-[0.65fr_1.35fr]">
                <form class="rounded-md border border-zinc-200 bg-white p-4 shadow-sm sm:p-5" @submit.prevent="form.post(route('access.permissions.store'), { preserveScroll: true, onSuccess: () => form.reset() })">
                ...
                <div class="overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm">
                    <div class="grid gap-0 divide-y divide-zinc-200">
                        <article v-for="permission in permissions" :key="permission.id" class="grid gap-3 p-4 md:grid-cols-[1fr_auto_auto] md:items-center">
                            <div>
                            ...
                            <button class="rounded-md border border-zinc-300 p-2 disabled:opacity-40" :disabled="permission.roles_count > 0" @click="router.delete(route('access.permissions.destroy', permission.id), { preserveScroll: true })">
